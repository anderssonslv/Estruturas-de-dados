...
// Main loop. Use p2 to remember previous p1
	...
	while (!found) {// A C D
  		if (found = strcmp(p1 -> item, newp -> item) >= 1) {
			//se cai nesse if a ordem do novo item é antes
			if (p1 == head) { 
				head = addfront(head, newp); 
				return(head);
		    }
...
// match not found before end of list so insert at end 
		if  (p1 -> next == NULL) { // ele testa se não há mais elementos adiante
			//nao passou no primeiro if, a ordem dele vai pro final
...
{
	start = addmiddle(start, newelement("A")); 
	start = addmiddle(start, newelement("C")); 
	start = addmiddle(start, newelement("D")); 
	start = addmiddle(start, newelement("B")); 
